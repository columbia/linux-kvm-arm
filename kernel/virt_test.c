...
static unsigned long hvc_test(void)
{
	...
	local_irq_save(flags);
	cc_before = read_cc();
	virt_test_call_hyp(NULL, (void*)HVC_NOOP);
	cc_after = read_cc();
	local_irq_restore(flags);
	ret = CYCLE_COUNT(cc_before, cc_after);
...
static unsigned long vmswitch_send_test(void)
{
	...
	local_irq_save(flags);
	cc_before = read_cc();
	ret = virt_test_call_hyp((void*)HVC_VMSWITCH_SEND, cc_before);
	if (ret)
...
static unsigned long vmswitch_recv_test(void)
{
	...
	local_irq_save(flags);
	cc_before = virt_test_call_hyp((void*)HVC_VMSWITCH_RCV);
	cc_after = read_cc();

	virt_test_call_hyp((void*)HVC_VMSWITCH_DONE);
	local_irq_restore(flags);
	ret = CYCLE_COUNT(cc_before, cc_after);
...
static unsigned long trap_profile_start(void)
{
	...
	cc_start = read_cc();
	virt_test_call_hyp((void*)TRAP_MEASURE_START);
	return ret;
}
...
{
	unsigned long ret = 0;
	virt_test_call_hyp((void*)TRAP_MEASURE_END);
	cc_end = read_cc();
...
static int __init virt_test_init(void)
{
	...
	/* Enable cycle counter for each CPU.
	 * Uncomment the the follwing line if you
	 * want to run virt-test on the host.
	 */
